...
$function = function(array $elephpants, array $reports) {
  array_map(function(Elephpant $elephpant) use($reports) {
    $elephpant->reports = array_filter(array_map(function(Report $report) use($elephpant) {
      if ($report->topic == $elephpant->favorite_topic) {
        return $report;
      }
    }, $reports));
  }, $elephpants);

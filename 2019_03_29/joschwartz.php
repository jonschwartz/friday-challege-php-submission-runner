...
$function = function($testData) {
  $visited = ['Suzi'];
  $totalWeight = 0;
  while (count($visited) < count($testData)) {
      $lastNode = $testData[end($visited)];
      $lowestWeight = 999;
      $lowestName = '';
      foreach ($lastNode as $name => $weight) {
        if (($weight < $lowestWeight) && (!in_array($name, $visited))) {
          $lowestWeight = $weight;
          $lowestName = $name;
        }
      }
...
echo (test($function) ? 'did it' : 'failed');